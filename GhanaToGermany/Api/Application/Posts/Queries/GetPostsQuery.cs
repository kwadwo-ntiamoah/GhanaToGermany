...
namespace Api.Application.Posts.Queries;

public record GetPostsQuery(string Requester, bool? GetOnlyUserPosts = false): IRequest<ErrorOr<List<PostModel>>>;

public class GetPostsQueryHandler(IUnitOfWork unitOfWork): IRequestHandler<GetPostsQuery, ErrorOr<List<PostModel>>> {
    public async Task<ErrorOr<List<PostModel>>> Handle(GetPostsQuery request, CancellationToken cancellationToken)
    {
        ...
        var posts = await unitOfWork.Post.GetPostsAsync();
